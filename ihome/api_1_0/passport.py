...
def register():
    ...
    # request.get_json方法能够帮助将请求体的json数据装换为字典
...
def login():
    ...
    if not re.match(r"1[34578]\d{9}", mobile):
        return jsonify(jsonify(errno=RET.PARAMERR, errmsg="手机号格式不正确"))
    ...
    if user is None or not user.check_passwork(password):
        # 出现错误, 累加错误次数
        try:
...
def check_login():
    ...
    # 如果session中的数据name名字存在, 则表示用户以登录.否则未登录
    if name is not None:
